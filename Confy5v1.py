...
def enviar_mensagem():
    global chave_aes, primeira_mensagem

    ip = ip_entry.get()
    mensagem = msg_entry.get()

    if not chave_publica_destinatario:
        messagebox.showwarning("Aviso", "Carregue a chave pública do destinatário.")
        return
    ...
    try:
        ...
        log_text.insert(tk.END, f"[ERRO] Falha ao enviar: {e}\n")
